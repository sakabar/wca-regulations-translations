...
- 2c1+) [追記] 登録要件は大会ウェブサイトに明言されていなければならない。
- 2c1++) [例] 例えば、大会運営チームは競技者に対し特定の期日までに登録確認メールの確認を要求したり、登録料の事前決済を要求したり、過去の大会の延滞登録料の支払いを要求したりすることができる。大会運営チームはこれらの要求の公平性を注意深く検討する必要がある。
