...
- A1a2++) [例] もしラウンドが累積方式で30分の制限時間であり、ある競技者の初め2回の試技が6:00とDNF (10:00) であった場合、3回目の試技の制限時間は 30:00 - 6:00 - 10:00 = 14:00 となる。

この別特な例に関しては、ある試技[?]の最後の結果のみがcountするという点に注意せよ。

つまり、3回目の試技
のペナルティを考慮しない元々の記録が13:59 [9f2を見よ]

なぜならfinal resultは13:59

であるが、3回目の試技が13:59 + 2 = 14:01 だった場合は制限を満たしていない


であることにに注意せよ
A1a2+++) [追記] もしある試技の結果がタイマーがリセットされるなどの事象により読み取り不能になった場合、ジャッジはWCA代理人にconsultし、
その試技にかかった時間の見積もる必要がある。この見積もり時間はtimelimitにcount towardsするが、indivisual試技のための公式記録のために使用してはならない
[TODO] 元の条文が消えているのは大丈夫なのか? issue確認
...
- A3c3++) [明示] 試技者はインスペクション時間中にタイマーの挙動を確かめたり、リセット(?)してはならない。

なぜならば、インスペクションは競技者が 1 度ソルブを開始した時に終了するからである。

もし競技者が

最初にそれ(どれ?)を始めた後にタイマーを止めた場合、それはソルブを終了したと考えられる(?) (see [Regulation A6](regulations:regulation:A6)を見よ)
インスペクションが開始してから15秒の間に行ったとしても


ソルブを開始すること ([Regulation A4d](regulations:regulation:A4d)を見よ)
...
る。例外：3x3x3片手競技と3x3x3足競技（参照： [規則 C1b](regulations:regulation:C1b)、[規則 D1b](regulations:regulation:D1b)）。
- A5b++) [明示] 験
もし競技者のパズルの一部がポップ(?外れる?)した場合(see [Regulation 5a](regulations:regulation:5a))、競技者は

そのパーツを探したりパズルに組み入れる(?)ように頼んではならないし、ジャッジは他の者が手伝けするのを防ぐ必要がある。

もし、競技者者以外の誰か(?)が競技者のために外れたパーツを拾った場合、ジャッジか競技者はその者にパーツをsolving stationに置き、これ以上の手伝けはしないようにお願いしてももいし、
...
- B2d+) [追記] 基本的に、競技者は最初、ジャッジの準備ができていることをindicateしていることを確認してから試技を始める。(see [Regulation B2a](regulations:regulation:B2a)。
もし競技者がin working orderにてタイマーを確認したいと思った場合は、競技者はそのphase(?)でタイマーを動かし始める前にジャッジに確認をとらなければならない(must)
ジャッジへの確認なしにタイマーを始めたりリセットしたりした場合は、ペナルティとして試技を失格とする(DNF)
...
- E2a1+) [推奨] もしジャッジが提出された解答を確認した際に、試技を特定する情報が見つからなかった場合はジャッジは回答用紙の両面を確認すべきである。
- E2b+) [明示] 競技者は終了時間より前に解答を提出することにより、早めに試技を終わらせることができる。
- E2b++) [追記] 競技者は既に競技が開始している最少手数競技に WCA 代理人の裁量により参加してもよい。この場合、途中参加した競技者の制限時間は残りの競技時間となる (すなわち、途中参加した競技者は他の競技者が60分の制限時間に達した時に競技を終了しなければならない)。例外: もしいずれかの競技者が既に試技を終了している場合や途中参加を希望する競技者にスクランブルに関する情報が知られているようになった危険性がある場合、競技者は途中参加してはならない。 FIXME 全角カッコ?()。
- E2c+) [明示] ジャッジは競技者のために公式の解答用紙を渡しているが、競技者は解答を別の紙に書いて提出してもよい。ただし、その紙はジャッジにより渡されたものでなければならない。(E3a)
- E2c1+) [例示] 大会登録IDとは、その大会に参加する競技者に割り当てられた、数字などの識別子である。大会によっては、大会登録IDを用いて記録用紙の経過を追い、記録を入力する。
- E2c1++) [追記] もし競技者がintendedな解答用紙に試技を特定する情報を制限時間までに書くことを忘れている場合、ジャッジの目の前でそれを追加してもよい。その場合、競技者は解答を提出している間にジャッジに尋ねなければならず、即座にそのジャッジの直接の監視の下で試技を特定する情報を(その他の変更を加えることなく)書かなければならない。一度競技者が解答の提出を完了したら(すなわち、解答用紙をジャッジに手渡した、ジャッジが許可して次の競技者の回収に移った、解答用紙をジャッジのテーブルに置いて立ち去った、など)、競技者はどのような情報も書き加えてはならない。もし競技者がジャッジに尋ねることなく制限時間後に何かを記入した場合、残りの試技は失格(DNF)となる。
- E2c1+++) [例示] 競技者は大会名、ラウンド、試技番号が記載された紙を提出する必要はない。しかし、提出した解答をWCAが追跡するのを手助けするためにそうしてもよい。
- E2c1++++) [追記] もしジャッジが(例えば、技試の採点中に)技試を特定する情報が無い解答に気付いた場合、ジャッジはDNSではなくDNFにするために、これを書いた競技者を探す必要がある。ただし、この時点で、その競技者は試技を特定する情報を書くことが許されてはならないので、競技者はDNFの結果を受け入れなければならないことに注意せよ。
- E2c2+) [例示] 単一の曖昧な操作列であるとは考えられない: 1つの明確に意図された解答の無い、複数の解答の列(?)、あいまいな記号を含む列 (例えば、「B」と「R」に似ているが、どちらともつかない文字)、書かれた順番を星印や矢印によってoutperformした…(?)、incorporatedしたpre-moves?、曖昧でない記号であると解釈できないシンボルの組み合わせ。
